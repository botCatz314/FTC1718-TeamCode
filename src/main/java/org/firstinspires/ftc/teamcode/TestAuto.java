...
public class TestAuto extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        SensorManager sensorServices = (SensorManager) hardwareMap.appContext.getSystemService(Context.SENSOR_SERVICE);
        if (sensorServices != null) {
            Sensor gyro = sensorServices.getDefaultSensor(Sensor.TYPE_GYROSCOPE);
        }
    ...
    private void TurnGyro2(double Turnamount, double speed){
       while(Turnamount - imu.getAngularOrientation(AxesReference.INTRINSIC, AxesOrder.ZYZ, AngleUnit.DEGREES).firstAngle >1 )
        if(Turnamount - imu.getAngularOrientation(AxesReference.INTRINSIC, AxesOrder.ZYZ, AngleUnit.DEGREES).firstAngle < 1
                && Turnamount - imu.getAngularOrientation(AxesReference.INTRINSIC, AxesOrder.ZYZ, AngleUnit.DEGREES).firstAngle > 1){
            rightMotor.setPower(powerOff);
            leftMotor.setPower(powerOff);
        }
        else if(Turnamount - imu.getAngularOrientation(AxesReference.INTRINSIC, AxesOrder.ZYZ, AngleUnit.DEGREES).firstAngle > 1){
            rightMotor.setPower(speed);
            leftMotor.setPower(-speed);
        }
        else if(Turnamount - imu.getAngularOrientation(AxesReference.INTRINSIC, AxesOrder.ZYZ, AngleUnit.DEGREES).firstAngle < 0){
            leftMotor.setPower(speed);
            rightMotor.setPower(-speed);
        }
        sleep(100);
